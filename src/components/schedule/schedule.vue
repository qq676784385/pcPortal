		...
		<div class="contList">
			...
			<table border="1" cellspacing="0" frame="box" rules="none" align="center">
				...
				<tr v-for='(visa,index) in visaListInfo'>
					...
					<!-- <td class="link" @click="gojindu(visa.VisaID,visa.Destination)">进度查询</td> -->
					<td class="link" v-tap="{methods:gojindu,info:visa}">进度查询</td>
...
<script type="text/javascript">
	export default{
		...
		methods:{
			gojindu(par){
				this.$root.set('UserVisaId',par.info.VisaID)
				this.$root.set('CountryName',par.info.Destination)
				this.$root.set('PassportNumber',par.info.PassportNumber)
				this.$root.set('visaType',par.info.VisaType)
				this.$root.set('Name',par.info.Name)
				this.$router.push({path: '/main/jindu'})
			...
			addMaterialFun(progress,visaId){
				...
				if(progress == 0){
					// 未填写信息
					this.$router.push({path: '/main/content/checkInfo'})
				}else{
					this.$router.push({path: '/main/content/info'})
				}
				this.$root.set("UserVisaId",visaId)
					...
					function(res){
						if(res.data.Code == 0){
								...
								for (var i = 0; i < res.data.Result.length; i++) {
									if(res.data.Result[i].VisaStatus>1 && res.data.Result[i].VisaStatus<7){
...
<style type="text/css" lang="scss" src='./css/schedule.scss'></style>

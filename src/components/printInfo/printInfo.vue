<template lang="html">
  <div class="applyTableInfo_print">
    <div class="printInfoBtn">
      <!-- <span class="pBtn" v-tap='{methods:printFun}'>
      ...
      <div id="tianbiao_scroller"> -->
        <div id="printCont" >
          <div class="applyTableInfo_print">
            ...
            <div id='page4' class="page1">
            </div>
            <div id='page5' class="page1">
            </div>
            <div id='page6' class="page1">
            </div>
            <div id='page7' class="page1">
            </div>
            <div id='page8' class="page1">
            </div>
          </div>
    ...
    <div class="column1">
      <div class="btn" v-tap="{methods:goChangeInfo}">
        返回修改
      </div>
      <div class="btn"  v-tap="{methods:submitFun}">
        确认提交
      </div>
      <div class="btn p_btn" v-tap='{methods:printFun}'>
          <img src="static/img/print.png">
          <span>打印</span>
      </div>
    </div>
...
export default {
  data() {
    return {
      // tanhao: false,
      showImg: "static/img/show.png",
      hideImg: "static/img/hide.png",
      showTip: false,
      area1: [],
      area2: [],
      level: 0,
  ...
  methods: {
    ...
    getGroupListInfo(num) {
      console.log(this.$root.crossLogin(), this.$store.state.app.token);
      var titleName = ''
      switch (num) {
        case 1:
          titleName = '基本信息';
      ...
      Div_Title.id = 'Title_' + num;
      Div_Title.innerHTML = "<span>" + titleName + "</span>";
      var img = document.createElement('img')
      img.setAttribute("src", "static/img/hide.png")
      Div_Title.appendChild(img)
      el.appendChild(Div_Title)
      Div_Title.addEventListener("click", function(e) {
        if ($(e.target).parent().hasClass("hidecont")) {
          $(e.target).parent().removeClass("hidecont")
          $(e.target).children('img').attr("src", "static/img/hide.png")
        } else {
          $(e.target).parent().addClass("hidecont")
          $(e.target).children('img').attr("src", "static/img/show.png")
        }
      ...
      this.$http.get(this.$store.state.app.host + 'api/Wechat/NeedGroupInfo', {
        ...
        function(res) {
          ...
          var el = document.getElementById('page' + num);
          var that = this
          this.createDom(el, res.data.Result.ShowTagInfo)
    ...
    dom(el, obj, level) {
      ...
      if (obj.FieldType == 5) { //数字
        Div_value = document.createElement('div'); //打印预览 有INPUT 改为DIV
        // if (window.localStorage.getItem('browser') != 'IE') {
      ...
      if (false && obj.Tag == 'JB000605' && window.localStorage.getItem('showmodel') == '1') { //身份证有效期至 怎加长期选项
      ...
      if (false && obj.Tag == 'JB000601' && window.localStorage.getItem('showmodel') == '1' && window.localStorage.getItem('CountryName') != '美国EVUS') { //身份证有效期至 怎加长期选项
        var Div_inputS = document.createElement('input')
        var Div_inputLabel = document.createElement('label')
        var Div_inputSpan = document.createElement('span')
        this.$root.addClass(Div_inputLabel, 'checkbox')
      ...
      this.$root.addClass(mask, 'mask')
      Div_info.appendChild(mask);
    },
    VerifyTypeFun(domKey, domValue, type) {
      ...
      if (type == 6) { //出发日期不能为今天及以前
        ...
        if (this.$root.verifyDay(domValue.innerHTML) <= 0) {
          ...
          // this.$root.showTip1(['如果您办理的是普通签证，请预留出至少30个工作日的签证办理时间；', '如果您办理的是绿通签证，请预留出至少10个工作日的签证办理时间。'])
          if (!this.$root.hasClass(domKey, 'red')) {
            this.$root.addClass(domKey, 'red')
          }
          if (!this.$root.hasClass(domValue, 'red')) {
    ...
    optionBlur(e, valueEl, childerDom, parentEl, level, tag, objOptions, boolan) {
      ...
      if (childerDom.length > 0) {
        ...
        if (tag == 'JT000501' || tag == 'JT000601' || tag == 'JT000716' || tag == 'LX003506' || tag == "JTXXL019" || tag == "JT000907" || tag == "JTXXL001" || tag == "JTUK0801" || tag == "JTUK0701" || tag == "LXUK0608" || tag == "LX003506") {
          ...
          // if (tag == 'JT000716' || tag == 'LX003506' || tag == "JT000907" || tag == "JTUK0701" || tag == "LXUK0608") {
          //   index = index + 1 //这两个OPTIONS不同 需要增加1个
          // }
    ...
    printFun() {
      if ($(".hidecont").length > 0) {
        $(".hidecont").each(function() {
          $(this).removeClass('hidecont')
          console.log($(this).children().eq(0).children("img"))
          $(this).children().eq(0).children("img").attr("src", "static/img/hide.png")
        })
      }
      document.body.innerHTML = document.getElementById("printCont").innerHTML
      window.print()
      location.reload()
    },
  },
  ...
  mounted: function() {
    ...
    var t = setTimeout(function() {
      that.getGroupListInfo(2)
    }, 100)
    var t = setTimeout(function() {
      that.getGroupListInfo(3)
    }, 200)


    var t = setTimeout(function() {
      that.getGroupListInfo(4)
    }, 300)
    if (window.localStorage.getItem('CountryName').indexOf('EVUS') == -1) {
      var t = setTimeout(function() {
        that.getGroupListInfo(5)
      }, 400)
    }
    var t = setTimeout(function() {
      that.getGroupListInfo(6)
    }, 500)
    var t = setTimeout(function() {
      that.getGroupListInfo(7)
    }, 600)
    var t = setTimeout(function() {
      that.getGroupListInfo(8)
    }, 700)
...
<style lang="scss" type="text/css" src='./css/printInfo.scss'>
